...
function drag(pointer) {
    ...
    if (last_x !== null) {
        ...
        let axis = new BrainBrowser.SurfaceViewer.THREE.Vector3(1, 0, 0).applyMatrix4(inverse).normalize();
        model.rotateOnAxis(axis, dy / 150);

        inverse.getInverse(model.matrix);
        axis = new BrainBrowser.SurfaceViewer.THREE.Vector3(0, 1, 0).applyMatrix4(inverse).normalize();
